...
def sweep(player_id_vec, w_vec, win_loss_df, rng, sigma):
    """
    Not really a sweep- we try n_players mutations, but the player that gets updated is
    chosen at random each time, so some players may be skipped or updated more than
    once.
    """
    n_chains, n_players = w_vec.shape
    assert player_id_vec.shape[0] == n_players, "Weight vector length does not match players"
...
def metropolis(player_id_list, restructured_df, n_samp, n_chains, burnin_sweeps, sweeps_per_samp):
    ...
    for iter in range(burnin_sweeps):
        w_vec = sweep(player_id_vec, w_vec, restructured_df, rng, sigma=0.1)
